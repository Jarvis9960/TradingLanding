...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 mx-auto grid w-full max-w-6xl flex-1 gap-12 pb-6 pt-12 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="flex flex-col justify-center gap-10">
          ...
          <div className="flex flex-wrap gap-4">
            <Button className="group rounded-full bg-[#f6d47d] px-8 py-6 text-base font-semibold text-black shadow-[0_20px_60px_rgba(246,212,125,0.35)] transition-transform duration-300 hover:-translate-y-1 hover:bg-[#ffe29b]">
              Join the Revolution
              <ArrowRight className="ml-2 transition-transform group-hover:translate-x-1" />
            </Button>
            <Button
              variant="outline"
              className="rounded-full border-white/20 bg-transparent px-8 py-6 text-base font-semibold text-white transition-all duration-300 hover:-translate-y-1 hover:border-white/40 hover:bg-white/10"
            >
              <Play className="mr-2 h-4 w-4" /> Watch the desk run
            </Button>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {metrics.map((metric, index) => (
              <div
                key={metric.label}
                className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 px-6 py-5 backdrop-blur"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <div className="absolute -right-10 top-1/2 h-24 w-24 -translate-y-1/2 rounded-full bg-[#f6d47d]/10 blur-3xl transition-opacity duration-500 group-hover:opacity-100" />
                <div className="relative space-y-2">
                  <div className="text-3xl font-semibold text-white sm:text-4xl">{metric.value}</div>
                  <p className="text-xs uppercase tracking-[0.45em] text-white/40">{metric.label}</p>
      ...
      <div className="relative z-20 mx-auto flex w-full max-w-6xl flex-wrap items-center justify-between gap-4 border-t border-white/5 pt-8 text-xs uppercase tracking-[0.4em] text-white/30">
        <div className="flex items-center gap-2">
          <span className="h-1.5 w-1.5 rounded-full bg-[#f6d47d]" />
          Global prop traders scaling together
        </div>
